...
void refresh();         //values that were changed are changed back to initial state
...
int main()
{
    ...
    {
        printf("\nFor function F%d\n",z);
        tab();              // the minimized function is calculated for function and it's compliment
        printf("\n\n");
        refresh();          //values are reset
        z++;
    }
    i=0;
    common();           //the combination with minimum terms is calculated
    answer();           //the function to be taken are displayed
    return 0;
}
...
{
    int i,x=savecount;
    printf("The combination that gives minimum number of product terms is :-");
    while(nof--)
    {
        printf("  F%d",nof+1);
        if(x%2==1)          //if the function is a compliment of the entered function
        {
            printf("'");
        }
        if(nof!=0)
...
void common()
{
    ...
    for(i=0;i<2*nof;i=i+2)          //values 0, 2, 4 are stored in arr which are indexes of functions in compare pointer array
    {
        arr[k]=i;
        k++;
    }
    ...
    while(counter<b)
    {
        int x=counter;
        counter2=0;
        for(i=0;i<nof;i++)      //checks which combination is to be used
        {
            a[i]=x%2;
            x=x/2;
        }
        for(i=0;i<nof;i++)      //all the terms in the combinations are stored in a single linked list
        {
            temp=compare[arr[i]+a[nof-1-i]]; //selects if the function or it's compliment is to be checked for in this combination
            while(temp!=NULL)       //the terms are stored in a separate linked list
            {
                int y;
        ...
        newtemp->right=NULL;
        end_loop(newroot);      //extra node is removed
        ...
        while(temp1!=NULL&&temp1->right!=NULL)      // not equal to null condition if the node next to temp1 gets deleted
        {
            ...
            while(temp2!=NULL)
            {
                if(strcmp(temp1->term,temp2->term)==0)      //if terms are same
                {
                    del(newroot,temp2);
                }
                temp2=temp2->right;
        ...
        temp1=newroot;
        while(temp1!=NULL)      //length of the union of the function of a combination is calculated
        {
            temp1=temp1->right;
            counter2++;
        }
        if(counter2<counter1)       //if length of union is less than that of previous one then their values are stored as minimum
        {
            counter1=counter2;
            savecount=counter;
        }
...
void del(struct node* first, struct node* temp)     //for deleting a node of a linked list
{
...
void tab()
{
    ...
    temp->right=NULL;
    end_loop(root) ;        //extra node is deleted
    arrange();      //various functions are called according to their needs
    store_minterms();
    int y;
    for(y=0;y<2;y++)        // tabulation is used twice once to find function them to find compliment of function
    {
        group1();
        disp();
        end_loop(root);
        ...
        end_loop(improot);
        collect(improot);       //various functions are called according to their need
        display_implicants();
        variables();
        implicants_table();
        other_implicants();
...
void refresh()      //changing variables to initial values
{
